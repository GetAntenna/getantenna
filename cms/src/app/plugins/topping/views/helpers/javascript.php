 ...
 */
	function event($object, $event, $observer = null, $options = array()) {
		$isObject = (
			...
			strpos($object, 'jQuery(') !== false || strpos($object, '"') !== false ||
			strpos($object, '\'') !== false
		);

        if ($isObject) {
            $b = "jQuery({$object}).bind('{$event}', function(){ {$observer} })";
        } elseif ($object[0] == '\'') {
            //TODO po co to?
            $b = "jQuery(". substr($object, 1) .").bind('{$event}', function(){ {$observer} })";
            ...
            // } else {
                $b = "jQuery('{$object}').bind('{$event}', function(){ {$observer} })";
                //$b = "Event.observe(\jQuery('{$object}'), '{$event}', function(event) { ";
                //$b .= "{$observer} });";
            // }
        }
