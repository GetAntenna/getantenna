<div id="contentWrapper" class="SF_left">
	<div id="contentHeader" class="SF_niceclear">
		<div id="contentHeaderMargin" class="SF_niceclear">
			<h1>Edit scrapbook event <span>(<a class="admin" href="<?php echo Router::url(array('action' => 'index')); ?>"><?php __('Cancel', false); ?></a>)</span></h1>
			<div id="sectionToolbar">
				...
				<a class="button" href="#eventMove" rel="shadowbox;height=198"><span><?php __('Move to recurring events', false); ?></span></a>
				...
				<button type="submit" class="readyButton" id="readyButtonTop" form="ScrapbookEventEditForm"><span><?php __('Save', false); ?></span></button>
			</div>
		</div>
	</div>
	<section id="content" class="SF_niceclear">
		...
		<?php echo $this->Form->create(null, array('url' => array('action' => 'edit', $eventId), 'enctype' => 'multipart/form-data')); ?>
			...
			<div id="fieldsWrapper">
				<fieldset>
					...
					<div id="autocompleteAddress" class="formRowInlineHalf formRow<?php if($form->error('ScrapbookEvent.address')) { echo ' formRowError'; } ?>">
						...
						<div<?php if($form->error('ScrapbookEvent.address')) { echo ' class="fieldError"'; } ?>><?php 
							...
							<a href="#eventLocation" rel="shadowbox;height=500;width=700;" id="eventAddressLookup" class="button buttonLocation<?php if($this->data['ScrapbookEvent']['lat'] != '') echo ' readyButton'; ?>"><img src="<?php echo $this->webroot; ?>images/icon_marker.png" /></a>
						</div>
				...
				<fieldset>
					...
					<?php 
						if($this->data['ScrapbookEvent']['picturen'] != '')
						{
					?>
		...
		<div id="eventDelete" class="SF_hidden">
			<div class="SBContent">
				<?php echo $this->Form->create("removeItem", array('url' => array('controller' => 'scrapbook_events', 'action' => 'delete'))); ?>
		...
		<div id="eventMove" class="SF_hidden">
			<div class="SBContent">
				<?php echo $this->Form->create("moveItem", array('url' => array('controller' => 'scrapbook_events', 'action' => 'move'))); ?>
					<?php echo $this->Form->input('id', array('label' => false, 'error' => false, 'type' => 'hidden', 'value' => $eventId)); ?>
		...
		<div id="eventAttach" class="SF_hidden">
			<div class="SBContent">
				<?php echo $this->Form->create("attachItem", array('url' => array('controller' => 'scrapbook_events', 'action' => 'attach'))); ?>
					<?php echo $this->Form->input('id', array('label' => false, 'error' => false, 'type' => 'hidden', 'value' => $eventId)); ?>
		...
		<script type="text/javascript">
			// Map constants
			var mapCenter = new google.maps.LatLng(<?php if($this->data['ScrapbookEvent']['lat'] != '' ) echo $this->data['ScrapbookEvent']['lat']; else echo Configure::read('Application.map.lat'); ?>, <?php if($this->data['ScrapbookEvent']['lon'] != '' ) echo $this->data['ScrapbookEvent']['lon']; else echo Configure::read('Application.map.lon'); ?>)
